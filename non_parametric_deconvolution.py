...
def modelfree_deconv(data, aif, dt, n_offset=1, hct=0.45,
    ...
    Adopted from https://github.com/liob/non-parametric_deconvolution
    ...
    t_max = np.argmax(I, axis=0).reshape(data.shape[1:])
    t_max = t_max * dt
    F = f * (100 * 60)
    ...
    # return {'flow': F, 'volume': V, 'mtt': T, 't_max': t_max}
    return np.squeeze(F), np.squeeze(V), np.squeeze(T), np.squeeze(t_max)
